...
>>>>>>> bd4ec71fe78f8f381b27d90f4dd83963a0d77392
    ...
    function update_group($gid)
    {
        ...
        $this->db->where('gid', $gid);
        if ($this->db->update('e_learn_group', $userdata)) {
    ...
    {
        $this->db->where('display_at', 'Result');
        $querys = $this->db->get('e_learnquiz_custom_form');

        $this->db->where('e_learnquiz_custom_form.display_at', 'Result');
        $this->db->where('e_learn_users_custom.uid', $uid);
        $this->db->join('e_learnquiz_custom_form', 'e_learnquiz_custom_form.field_id=e_learn_users_custom.field_id');
        $query = $this->db->get('e_learn_users_custom');
        return ($querys->num_rows() - $query->num_rows());
    }
